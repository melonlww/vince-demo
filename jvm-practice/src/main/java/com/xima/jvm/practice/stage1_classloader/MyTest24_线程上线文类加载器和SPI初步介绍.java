 ...
 *
 * 线程上下文加载器是从JDK 1.2开始引入的，类Thread的getContextClassLoader()与setContextClassLoader(ClassLoader cl)
 * 分别用来获取和设置上下文加载器
 *
 * 如果没有通过setContextClassLoader(ClassLoader cl)进行设置的话，线程将继承其父线程的上下文类加载器。
 * Java应用运行时的初始线程的上线文类加载器是系统类加载器，在线程中运行的代码可以通过该类加载器来加载类与资源
 *
 * 线程上下文类加载器的重要性：
 *
 ...
 * 双亲委托模型
 *
 * 线程上线文类加载器其实当前线程的Current Classloader
 *
 ...
 * 来源的java包，这样传统的双亲委托模型就无法满足SPI的请求。而通过给当前线程设置上下文加载器，就可以由设置的上下文加载器
 * 来实现对于接口实现类的加载
 */
public class MyTest24_线程上线文类加载器和SPI初步介绍 {
